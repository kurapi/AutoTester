# encoding: UTF-8
# filename: bill/lib/helpers/bill_yaml_helper.rb
...
def load_steps_lib
  ...
  steps_yaml_files = File.expand_path(File.join(Dir.pwd, 'bill', 'steps', "*.yml"))
...
def load_features_lib
  ...
  features_yaml_files = File.expand_path(File.join(Dir.pwd, 'bill', 'features', "*.yml"))
    ...
    features.each do |features_suite_name, features_suite_hash|
      ...
      features_suite_hash.each do |feature_name, feature_steps_list|
        ...
        feature_steps_list.each do |step|
          ...
          features_lib_hash[features_suite_name][feature_name] << step_hash.clone
        end
      end
...
def load_testcase_yaml_file(testcase_yaml_file_path)
  ...
  YAML.load_file(testcase_yaml_file_path).each do |testcase_name, features_suite|
    ...
    features_suite.each do |feature|
      features_suite_name, feature_name,feature_name_desc, run_times = feature.strip.split('|')
      ...
      run_times.times.each do |t|
        ...
        feature_hash['feature_name_desc'] = feature_name_desc
        feature_hash['features_suite_name'] = features_suite_name
