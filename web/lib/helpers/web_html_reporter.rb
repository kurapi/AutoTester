...
def set_htmlReport(testcase_hash)
  ...
  testcase_hash['features_suite'].each do |n|
    ...
    @AcountFeatures+=1 if n['features_suite_name']=='AccountFeatures'
    @PersonFeaturesPass+=1 if n['features_suite_name']=='PersonFeatures' and n['feature_ruslt']=='passed'
    @GroupFeaturesPass+=1 if n['features_suite_name']=='GroupFeatures' and n['feature_ruslt']=='passed'
    @AcountFeaturesPass+=1 if n['features_suite_name']=='AccountFeatures' and n['feature_ruslt']=='passed'
  end
  ...
  content+="<TR><TD>通过率</TD><TD>#{sprintf("%.2f",(@PersonFeaturesPass*100).to_f/@PersonFeatures).to_s+"%"}</TD><TD>#{sprintf("%.2f",(@GroupFeaturesPass*100).to_f/@GroupFeatures).to_s+"%"}</TD><TD>#{sprintf("%.2f",(@AcountFeaturesPass*100).to_f/@AcountFeatures).to_s+"%"}</TD></TR>"
  content+='</TBODY></TABLE>'
  ...
  testcase_hash['features_suite'].each do |n|
    ...
    n['feature_steps'].each do |m|
      ...
      content+="<TR><TD>#{m['step_print']}</TD><TD>step_print</TD></TR>" if m['step_print']!=nil
