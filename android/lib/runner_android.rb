# filename: android/lib/runner_android.rb
require "pathname"
...
def run_test(options)
  ...
  if app_type == "android"
    android_devices = capability.delete(:simulators).delete(:android_devices)
    devices_list = initialize_android_simulators(android_devices)
  else
  ...
  devices_list.each do |device|
    ...
    begin
      ...
      if File.exists? testcase_file
        # e.g. /Users/Leo/MyProjects/AppiumBooster/android/testcases/login.yml
        testcase_files = testcase_file
      elsif Pathname.new(testcase_file).absolute?
        # e.g. /Users/Leo/MyProjects/AppiumBooster/android/testcases/*.yml
        testcase_files = testcase_file
      elsif testcase_file.include? File::SEPARATOR
        # e.g. android/testcases/*.yml
