# filename: android/lib/helpers/android_simulator_helper.rb
...
def delete_existed_simulators(device_type, runtime)
  ...
  devices.each do |device|
    $LOG.info "Remove android simulator: #{device_type} (#{runtime}) (#{device[0]})".green
    `xcrun simctl delete #{device[0]}`
  end
end

def recreate_android_simulator(device_type, runtime)
  ...
  $LOG.info "Create android simulator: #{device_type} (#{runtime}) (#{simulator.strip})".green
end
